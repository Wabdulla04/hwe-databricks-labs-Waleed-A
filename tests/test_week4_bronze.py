...
def test_stores_insert_overwrite(spark):
    ...
    assert len(downtown) == 1 and len(airport) == 1
...
def test_categories_insert_overwrite(spark):
    ...
    assert len(fiction) == 1 and len(sci_fi) == 1 and len(space_opera) == 1
...
def test_online_orders_merge(spark):
    ...
    assert len(row) == 1 and row[0].customer_email == "alice@example.com"
...
def test_instore_orders_merge(spark):
    ...
    assert len(row) == 1 and row[0].cashier_name == "Bob Jones"
...
def test_categories_hierarchy_preserved(spark):
    ...
    assert fiction.parent_category_id == None and sci_fi.parent_category_id == "fiction" and space_opera.parent_category_id == "sci_fi"
...
def test_instore_orders_nullable_email(spark):
    ...
    assert row[0].customer_email == None
...
def test_merge_updates_existing_rows(spark):
    ...
    assert len(rows) == 1 and rows[0].customer_email == 'alice_updated_email@example.com'
